(function (angular) {
    ...
    var app = angular.module('eNutri.menus',['backend.utils','ngRoute','ngCookies', 'backend.utils', 'ngRoute','pascalprecht.translate','nvd3']);

    app.controller('MenuController', ['BEutil','$scope','$location', '$cookies', '$mdSidenav', '$translate','$mdDialog','$q',
        function MenuController(BEutil,$scope, $location, $cookies, $mdSidenav, $translate, $mdDialog) {
            ...
            // To update the menu, just emit a corresponding event
            $scope.$on('menu_update_needed', function(event) {
                BEutil.BEWait(false,true,false).then(() => $scope.$evalAsync(updateMenuItemsInternal));
            });

            // like this :)
            $scope.$emit('menu_update_needed');

            // Expose different menu items based on the user's login status, if he is a researcher,...
            function updateMenuItemsInternal()
            {
                self.menus=[];
                console.debug("MENU LOGIN: "+BEutil.signed_in);

                // Is the user signed in?
                if(BEutil.signed_in)
                {
                    // Is he (likely) a researcher?
                    if(BEutil.BEUser.get("group")=="researcher")
                    {
                        self.menus.push({
                            url: 'research',
                            name: 'research',
                        ...
                        // Does the user have any reports available?
                        if(BEutil.BEUser.get("state").get("FFQCompleteCount")>0)
                        {
                            self.menus.push({
                                url: 'reports',
                                name: 'reports',
            ...
            function showHelp (ev) {
                $mdDialog.show({
                    ...
                    locals: {selectedLanguage:self.selectedLanguage},
            ...
            function DialogController($scope, $mdDialog, BEutil) {
